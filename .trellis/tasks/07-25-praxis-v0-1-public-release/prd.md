...
- 五个知识入口除在“知识”页面内展示外，也由共享页眉中的“知识”披露菜单直接提供；桌面端支持悬浮展开，
  键盘与触屏支持显式展开。菜单必须保留“知识总览”入口，并以静态链接呈现五个 section，不增加三级技术目录。
  JavaScript 只允许增强桌面悬浮、外部关闭与 Escape 行为；关闭 JavaScript 后，原生披露控件仍能展开并完成导航。
...
- [x] AC11：一级导航提供“知识、项目、旅程、关于”，不提供独立“学习路径”，也不再把“博客、笔记、日志”作为
      一级栏目；品牌标识仍可返回首页。“知识”提供一个包含知识总览与五个固定 section 的披露菜单，其余三个一级入口
      仍是无子导航的直接链接。
