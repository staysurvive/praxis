...
Markdown/MDX under the repository root `content/` is the authored source of truth. Astro's official typed content
loader/collection boundary owns discovery and schema validation. Pages and components never parse raw frontmatter.
...
This contract applies to every authored Markdown/MDX entry and to the build-time Praxis Practice Heatmap. It was
...
- Authored input is frontmatter plus Markdown/MDX under `content/`; `contentId` is immutable and `slug` is URL-facing.
